...
## 예제 코드
미로 생성의 문제에 추상 팩토리 패턴을 사용해 봅시다. `MazeFactory` 클래스는 미로의 구성요소들을 생성합니다. 이는 방, 벽과 방 사이의 문을 만들어 냅니다. 이를 위해 파일에 정의된 미로에 대한 계획을 읽어 해당 미로를 구성할 수도 있을 것이고, 프로그램에서 임의적으로 미로를 만들 수도 있을 것입니다. `MazeFactory` 객체를 인자로 받아서 미로는 만드는 프로그램이기 때문에 프로그래머는 방, 벽, 문에 대한 클래스를 명시해야 합니다.
```c++
class MazeFactory {
  public:
    MazeFactory();
...
```
`CreateMaze()` 멤버 함수는 다른 멤버 함수를 이용해서 방들 사이에 문이 있는 방 두 개짜리 미로를 만들어 냅니다. `CreateMaze`는 클래스 이름을 하드코딩해놨기 때문에 서로 다른 구성요소를 가지고 미로를 만들어 내기가 힘듭니다.

아래의 코드는 `MazeFactory`를 매개변수로 받도록 하여 그 문제를 해결한 `CreateMaze` 함수입니다.
```c++
...
```
`EnchantedMazeFactory`를 `MazeFactory`에서 서브클래싱한 후, 멤버 함수를 재정의하여 `Room`, `Wall`을 상속하는 다른 서브클래스의 인스턴스를 반환하게 만듭니다.
```c++
...
```
만약 폭탄이 장착된 방을 만들고 싶다면, 방에 폭탄이 있는지 추적하고 관리하는 클래스를 `Room` 클래스의 서브클래스로 만들면 됩니다. 또한 방의 폭탄이 터진 후 벽에 손상이 갔을 때 벽의 모습이 바뀌도록 하려면, `Wall`의 새로운 서브클래스를 만들면 됩니다. 이를 `RoomWithBomb` 클래스와 `BombedWall` 클래스로 정의합니다.
```c++
...
```
폭탄이 들어 있을 수 있는 간단한 미로를 만들려면, 그냥 `BombedMazeFactory`를 `CreateMaze`에 넘겨서 호출하면 됩니다.
```c++
  MazeGame game;
...
```
마법이 걸린 미로를 만들려면, 마찬가지로 `EnchantedMazeFactory`의 인스턴스를 `CreateMaze` 함수에 넘기면 됩니다.

`MazeFactory`는 단순히 팩토리 메서드들이 모인 집합체입니다. 이렇게 하는 것이 추상 팩토리 패턴을 구현하는 가장 일반적인 방법입니다. 또한, `MazeFactory`는 추상 클래스가 아니므로 `AbstractFactory`나 `ConcreteFactory`로 사용할 수도 있는데요, 추상 팩토리 패턴을 간단하게 응용하는 또 하나의 일반적인 구현 방법으로 보시면 되겠습니다. `MazeFactory`는 팩토리 메서드를 모두 정의하는 구체 클래스이므로 변경할 연산을 재정의하고 서브클래스를 정의하여 새로운 `MazeFactory`를 쉽게 만들 수 있습니다.

`CreateMaze()` 연산은 방향을 정의하기 위해 방에 정의된 `SetSide()` 연산을 이용하였습니다. `BombedMazeFactory`로 방을 생성했다면 미로는 `RoomWithABomb` 객체와 `BombedWall` 객체로 만들어집니다. `RoomWithABomb` 클래스는 `BombedWall` 멤버를 접근해야 하므로 `Wall*`에 대한 매개변수를 `BombedWall*`에 대한 매개변수로 변환해야 합니다. 이와 같은 타입 변환은 모든 벽들이 `BombedMazeFactory` 내에서만 쓰고 있으며, `BombedWall`은 `Wall`의 서브클래스이므로 가능합니다.